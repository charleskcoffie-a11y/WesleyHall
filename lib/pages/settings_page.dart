...
class _SettingsPageState extends State<SettingsPage> {
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                      ...
                      children: [
                        Expanded(
                          child: Column(children: [
                            _field(manager, 'Manager / Authorized Officer'),
                            const SizedBox(height: 12),
                            _field(title, 'Position / Title'),
                            SwitchListTile(
                              contentPadding: EdgeInsets.zero,
                              title: const Text(
                                  'Automatically print manager signature on contracts'),
                              value: autoSignature,
                              onChanged: (v) =>
                                  setState(() => autoSignature = v),
                            ),
                          ]),
                        ),
                  ...
                  _card('Rental Time & Deposit Rules', [
                    Row(children: [
                      Expanded(child: _field(hours, 'Standard Rental Hours')),
                      const SizedBox(width: 12),
                      Expanded(child: _field(setup, 'Setup Minutes Before')),
                      const SizedBox(width: 12),
                      Expanded(child: _field(cleanup, 'Cleanup Minutes After')),
                    ]),
                    const SizedBox(height: 12),
                    Row(children: [
                      Expanded(child: _field(extraRate, 'Additional Hour Rate (\$)')),
                      const SizedBox(width: 12),
                      Expanded(child: _field(deposit, 'Booking Deposit (%)')),
                      const SizedBox(width: 12),
                      Expanded(child: _field(damage, 'Refundable Damage Deposit (\$)')),
                    ]),
                    const SizedBox(height: 8),
                    Text(
                      'The planner blocks the hall from setup/access time until cleanup/vacate time.',
                      style: Theme.of(context).textTheme.bodySmall,
                    ),
                  ]),
                  const SizedBox(height: 14),
                  _card('Rental Terms & Conditions', [
                    ...b.terms.map(
                      (t) => ListTile(
                        contentPadding: EdgeInsets.zero,
                  ...
                  const SizedBox(height: 14),
                  _card('Hall Spaces & Service Rates', [
                    ...b.spaces.map(
                      (h) => ListTile(
                        contentPadding: EdgeInsets.zero,
                        leading: const Icon(Icons.meeting_room_outlined),
                        title: Text(h.name),
                        trailing: Text('\$${h.baseRate.toStringAsFixed(2)}'),
                      ),
                    ),
                    const Divider(),
                    ...b.services.map(
                      (s) => ListTile(
                        ...
                        title: Text(s.name),
                        trailing: Text(
                            '\$${s.price.toStringAsFixed(2)}${s.pricingType == 'hourly' ? '/hr' : ''}'),
                      ),
                    ),
                  ]),
                ],
  ...
  Widget _card(String titleText, List<Widget> children) {
    return Card(
      child: Padding(
